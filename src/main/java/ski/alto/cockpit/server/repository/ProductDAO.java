...
public class ProductDAO {
	...
	private static final int MADONNA_RESORT_ID = 40;
...
//		 select distinct rc.id , rc.display_name
//		 from resort_ticket_prices rtp 
//		 JOIN resort_seasons rs on rtp.season_id = rs.id
//		 JOIN resort_categories rc on rtp.category_id = rc.id
//		 JOIN resort_tickets rt on rtp.ticket_id = rt.id 
//		 JOIN resort_ticket_groups rtg on rtg.id = rt.group_id
//		 where rs.resort_id = 40
...
//	}
	...
	public List<ProductWithCategoryDTO> getProductsBySeasonId(Integer seasonId, List<CategoryWithPriceDTO> categories) {
		...
		jdbcTemplate.query(
				"select list_price_cents, list_price_currency, rs.display_name as resort_season_name," +
						...
						" order by resort_ticket_id, resort_category_name;",
				new RowCallbackHandler() {
					@Override
					public void processRow(ResultSet rs) throws SQLException {
	...
	private String get_category_id_list (List<CategoryWithPriceDTO> categories) {
		StringBuffer to_return = new StringBuffer();
		boolean is_first = true;
		to_return.append("(");
		for(CategoryWithPriceDTO current_cat : categories) {
			if(!is_first) {
				to_return.append(",");
			}
			to_return.append(current_cat.getCategoryId().intValue());
			is_first = false;
		}
		to_return.append(")");
