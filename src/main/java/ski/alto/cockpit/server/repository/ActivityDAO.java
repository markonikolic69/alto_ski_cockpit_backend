...
public class ActivityDAO {
	...
	public List<ActivityDTO> getAllDTACardActivitiesForResortIdForToday(String todays_date, Integer resort_id, String ownership) {
		List<ActivityDTO> activities = null;
		logger.info("--> getAllDTACardActivitiesForResortIdForToday, todays_date = " + todays_date + ", resort_id = " + 
				resort_id + ", ownership = " + ownership);
		...
		//		String query_string = "select ua.resort_name resort_name, sc.first_name first_name, sc.last_name last_name, sc.card_number card_number, TO_CHAR(ua.activity_date,'YYYY-MM-DD HH24:MI:SS') activity_date, "
		...
		//		
		//		
		//		logger.info("getAllDTACardActivitiesForResortIdForToday, query_string = " + query_string );
		//
		//		activities =  jdbcTemplate.query(
		//				query_string,
		...
		if(resort_id != -1 && ownership != null) {

			//!-1, !null
			activities =  jdbcTemplate.query(
					query_string,
					(rs, rowNum) -> { 
						ActivityDTO resultRow = new ActivityDTO();
						resultRow.setResort_name(rs.getString("resort_name"));
						resultRow.setCard_number(rs.getString("card_number"));
						resultRow.setActivity_date(rs.getString("activity_date"));
						resultRow.setReader_name(rs.getString("reader_name"));
						resultRow.setBlock_number(rs.getString("block_number"));
						resultRow.setLift_number(rs.getString("lift_number"));
						resultRow.setLift_name(rs.getString("lift_name"));
						resultRow.setReader_number(rs.getString("reader_number"));
						resultRow.setFirst_name(rs.getString("first_name"));
						resultRow.setLast_name(rs.getString("last_name"));
						resultRow.setUser_id(rs.getInt("user_id"));
						return resultRow;
					}, 
					todays_date, resort_id, ownership );
		}else {
			if(resort_id != -1 && ownership == null) {
